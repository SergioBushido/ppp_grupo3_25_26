...
- [ ] **Issue #34 - Configurar fichaje por geolocalización flexible por empleado**
  ...
  - *Estado actual:* Implementación funcional inicial completada y reforzada en cliente: migración base aplicada, fichaje con geolocalización funcionando tras ajuste RLS, trazabilidad visible en admin, UI responsive corregida y validación endurecida en `attendanceService` para reconsultar la política/centro vigentes del empleado justo antes de fichar. También se corrigió la comprobación de coordenadas para no depender de checks falsy en latitud/longitud. Queda recomendable en una iteración posterior reintentar encapsular la validación crítica en RPC o backend cuando el despliegue SQL pueda hacerse con mayor fiabilidad y cerrar la batería manual completa.
  - *Criterios de aceptación:* Pendiente de cierre final. El admin ya puede configurar el modo de fichaje y la app dispone de validación funcional inicial con trazabilidad; falta ejecutar y documentar la batería manual prevista (`anywhere`, `assigned_center`, `manual_only`, permiso denegado y fuera de radio`) y decidir si la validación definitiva queda consolidada en cliente o se vuelve a mover a backend.
